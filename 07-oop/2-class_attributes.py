...
#====no need for [self] to use class attributes for objects contrly to instance attributes
#we can use class attributes inside for all objects of the class
class Member:

  not_allowed_names = ["Hell", "Shit", "Baloot"] #class attribute 1

  users_num = 0 #class attribute 2
  ...
  def full_name(self):
    ...
    if self.fname in Member.not_allowed_names: # we can use class attribute1 inside methods normaly for all objects
  ...
  def delete_user(self):
    ...
    Member.users_num -= 1  # same thing for class attribute2
